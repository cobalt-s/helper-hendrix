// popup.js
import { storeUserPokemon } from "./storage.js";
import {
  setStore,
...
// UI Helpers
function showOpenAIKeyUI() {
  document.getElementById("openaiKeyContainer").style.display = "block";
  document.getElementById("canvasKeyContainer").style.display = "none";
  document.getElementById("chatContainer").style.display = "none";
}
function showCanvasKeyUI() {
  document.getElementById("openaiKeyContainer").style.display = "none";
  document.getElementById("canvasKeyContainer").style.display = "block";
  document.getElementById("chatContainer").style.display = "none";
}
function showChatUI() {
  document.getElementById("openaiKeyContainer").style.display = "none";
...
function renderMyPokemon() {
  const container = document.getElementById("myPokemonContainer");
  container.innerHTML = "";
...
function renderPokemonShop() {
  ...
  storePokemon.forEach((p) => {
    ...
    price.textContent = `${p.price} pts`;
    const btn = document.createElement("button");
    btn.textContent = "Buy";
    btn.onclick = async () => {
      if (totalPoints >= p.price) {
        buyPokemon(p);
        const owned = await fetchMyPokemon();
        storeUserPokemon(owned);
        renderPokemonShop();
        renderMyPokemon();
...
chrome.storage.local.get(
  ["openai_api_key", "canvas_api_key", "totalPoints"],
  ({ openai_api_key, canvas_api_key, totalPoints: pts }) => {
    if (!openai_api_key) {
      showOpenAIKeyUI();
    } else if (!canvas_api_key) {
...
async function fetchCanvasTodos() {
  ...
  try {
    const coursesRes = await fetch("https://canvas.uw.edu/api/v1/courses", {
  ...
  const todos = await res.json(),
    container = document.getElementById("canvasTodos");
  ...
  todos.forEach((item) => {
    ...
    div.append(cb, document.createTextNode(item.title));
...
async function fetchCalendarEvents() {
  ...
  try {
    ...
    if (assignmentEvents.length === 0) {
      console.log(
        "No calendar assignment events, fetching via upcoming_events endpoint..."
      );
      try {
        ...
        console.log(
          "Assignment events from upcoming_events:",
          assignmentEvents
        );
      } catch (err) {
  ...
  const res = await fetch("https://api.openai.com/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: "system",
          content: "You are Hendridx, a study assistant. And you bark",
        },
...
document.getElementById("sendBtn").onclick = async () => {
  ...
  try {
    out.textContent = await sendToChatGPT(input);
    generateTasks();
  } catch (e) {
...
document.querySelectorAll(".shop-item").forEach((img) => {
  img.onclick = () => {
    ...
    if (totalPoints >= cost) {
      adjustPoints(false);
      alert(`Purchased for ${cost}`);
    } else alert(`Need ${cost}`);
  };
});
