...
async function sendToChatGPT(text) {
  const res=await fetch('https://api.openai.com/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${OPENAI_API_KEY}`},body:JSON.stringify({model:'gpt-3.5-turbo',messages:[{role:'system',content:'You are Hendridx, a study assistant.'},{role:'user',content:text}],max_tokens:300,temperature:0.7})});
